...
| 变更涉及的领域 | 目标文档 |
|---------------|---------|
| 着色器语法、代码生成、WGSL 输出 | `docs/code-generation.md` |
| 变量（attribute, uniform, varying） | `docs/variables.md` |
| 类型（vec, mat, float, int） | `docs/types.md` |
| 数学函数（sin, cos, pow, clamp） | `docs/math-functions.md` |
| 纹理采样、sampler | `docs/texture.md` |
| 控制流（if, return, select） | `docs/control-flow.md` |
| 向量运算（dot, cross, normalize） | `docs/vector-ops.md` |
| 着色器入口（vertex, fragment, func） | `docs/shader-entry.md` |
| API 导出、index.ts | `docs/api.md` |
| 架构、构建流程 | `docs/architecture.md` |
...
| 文档文件 | 内容要求 |
|---------|---------|
| `docs/architecture.md` | 系统架构概述、代码生成流程、模块依赖关系 |
| `docs/code-generation.md` | WGSL 代码生成、buildShader、IElement 接口 |
| `docs/variables.md` | 变量声明 API：attribute, uniform, varying, let, var, struct, array |
| `docs/types.md` | 类型系统：标量、向量、矩阵类型 |
| `docs/math-functions.md` | 数学函数：三角、指数、通用函数 |
| `docs/texture.md` | 纹理采样、sampler 类型、texture 函数 |
| `docs/control-flow.md` | 控制流：if_, return_, select |
| `docs/vector-ops.md` | 向量运算：dot, cross, normalize, reflect |
| `docs/shader-entry.md` | 着色器入口：vertex, fragment, func, transform |
| `docs/api.md` | 完整 API 参考、导出列表 |
| `docs/examples.md` | 示例代码、使用模式 |
...
### 文档更新
- ✓ 更新 docs/code-generation.md（更新了 XXX 章节）
- ✓ 创建 docs/texture.md（新文档）
...
- **中文输出**：所有文档使用简体中文，保留 WGSL, WebGPU, TypeScript, API 等技术术语为英文
