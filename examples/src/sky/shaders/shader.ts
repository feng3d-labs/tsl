...
export const vertexShader = vertex('main', () =>
{
    ...
    assign(vSunE, float(EE).multiply(max(0.0, float(1.0).subtract(exp(float(-1.0).multiply(float(cutoffAngle).subtract(acosClamped).divide(float(steepness))))))));
...
export const fragmentShader = fragment('main', () =>
{
    ...
    const inverse = var_('inverse', float(1.0).divide(cos(zenithAngle).add(float(0.15).multiply(pow(float(93.885).subtract(zenithAngle.multiply(float(180.0)).divide(float(pi))), float(-1.253))))));
    ...
    const uv = var_('uv', vec2(phi, theta).divide(vec2(float(2.0).multiply(float(pi)), float(pi))).add(vec2(0.5, 0.0)) as any);
